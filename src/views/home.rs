...
pub fn Home() -> Element {
    ...
    rsx! {
        div { class: "drawer lg:drawer-open min-h-screen bg-base-100",
            input { id: "room-navigation", r#type: "checkbox", class: "drawer-toggle" }
                ...
                header { class: "navbar border-b border-base-300 bg-base-100 px-4 sm:px-8",
                    ...
                    div { class: "min-w-0 flex-1",
                        if !resource_errors().is_empty() {
                            div { class: "max-w-xl truncate text-sm text-error", title: resource_errors().join("\n"),
                    ...
                    div { class: "flex-none",
                        if let Some(Ok(account)) = &*account.read() {
                            ...
                            button { class: "btn btn-ghost btn-square", title: "修改密码", onclick: move |_| is_change_password_open.set(true),
                                span { class: "icon-[mdi--lock-reset] size-5" }
                            }
                            button { class: "btn btn-ghost btn-square", title: "退出登录", onclick: move |_| logout_action.call(),
                                span { class: "icon-[mdi--logout-variant] size-5" }
                            }
                        } else {
                            button { class: "btn btn-ghost btn-square", title: "登录管理", onclick: move |_| is_login_open.set(true),
                        ...
                        div { class: "flex flex-wrap items-start justify-between gap-4",
                            div {
                                div { class: "group mb-2 flex items-center gap-2",
                                    if room().is_locked { span { class: "icon-[mdi--lock] size-4 text-base-content/60" } }
                                    if room().is_live {
                                        span { class: "status status-success status-sm" }
                                    }else{
                                        span { class: "status status-neutral status-sm" } span { class: "text-xs text-base-content/55", "[离线]" }
                                    }
                                    h1 {
                                        class: "text-2xl font-bold",
                                        contenteditable: if can_manage() { "plaintext-only" }else{ "false" },
                                        oninput: move |e| { if can_manage() { title_draft.set(e.value()); } },
                                        onblur:  move |_| async move {
                                            if can_manage() && title_draft() != room().title {
                                                update_room_action.call(room().stream_key.clone(),api::rooms::UpdateRoomRequest {  title: Some(title_draft()), lock_password:None }).await;
                                                rooms.restart();
                                            }
                                    },
                                        "{room().title}"
                                    ...
                                    if can_manage() {
                                        button { class: "btn btn-ghost btn-square btn-sm opacity-0 transition-opacity group-hover:opacity-100", title: if room().is_locked { "解锁" } else { "上锁" }, onclick: move |_| {
                                            update_room_action.call(room().stream_key.clone(),api::rooms::UpdateRoomRequest {  title: None, lock_password:None });
                                            lock_override.set(Some(!room().is_locked));
                                        }, span { class: if room().is_locked { "icon-[mdi--lock] size-4" } else { "icon-[mdi--lock-open-variant-outline] size-4" } } }
                                    }
                                }
                                p { class: "mt-1 text-sm text-base-content/60", "房主: {room().owner_username} · {room().viewer_count} 人观看" }
                            }
                            div {
                                    ...
                                    div { class: "dropdown dropdown-end dropdown-hover",
                                        ...
                                        div { class: "card card-sm dropdown-content bg-base-100 rounded-box z-1 w-64 shadow-sm",
                                            div {
                                                class: "card-body",
                                                p {
                                                    PublishAddress { label: "SRT", address: info.srt_url.clone() }
                                                    PublishAddress { label: "RTMP", address: info.rtmp_url.clone() }
                                ...
                                if let Some(play_url) = play_url() {
                                    div { class: "dropdown dropdown-end dropdown-hover",
                                        ...
                                        ul { class: "dropdown-content menu z-20 border border-base-300 bg-base-100 p-2 shadow-xl",
                                            div {
                                                class: "join",
                                                input { r#type: "radio", class: "join-item btn btn-sm", name: "play-mode", checked: play_mode() == "srt", value: "srt", "aria-label": "SRT", onchange: move |_| play_mode.set("srt") },
                                                input { r#type: "radio", class: "join-item btn btn-sm", name: "play-mode", checked: play_mode() == "rtmp", value: "rtmp", "aria-label": "RTMP", onchange: move |_| play_mode.set("rtmp") },
                                                input { r#type: "radio", class: "join-item btn btn-sm", name: "play-mode", checked: play_mode() == "http-flv", value: "http-flv", "aria-label": "HTTP-FLV", onchange: move |_| play_mode.set("http-flv") }
                                            }
                                            for p in PLAYERS {
                                                li {
                                                    a { onclick: {let play_url = player_url(p.0,&play_url); move |_| {document::eval(&format!("window.open('{play_url}');console.log('{play_url}');"));}},
                                                        i {
                                                            class: "size-5 bg-cover",
                                                            background_image: Some(format!("url(\"https://emby-external-url.7o7o.cc/embyWebAddExternalUrl/icons/{}\")", p.2))
                                                        }
                                                        "{p.1}"
                                                    }
                                                }
            ...
            div { class: "drawer-side",
                label { r#for: "room-navigation", aria_label: "关闭直播间列表", class: "drawer-overlay" }
                aside { class: "flex min-h-full w-72 flex-col border-r border-base-300 bg-base-200",
                    div { class: "flex items-center justify-between px-5 py-5",
                        ...
                        button { class: "btn btn-ghost btn-square btn-sm", title: "创建直播间", disabled: !is_administrator(), onclick: move |_| {
                    ...
                    ul { class: "menu w-full gap-1 px-3",
                        ...
                        if let Some(Ok(rooms)) = &*rooms.read() {
                            ...
                            li {
                                span { class: "px-4 py-3 text-sm text-base-content/55", "正在加载直播间..." }
                            }
                        }
                    }
                }
        ...
        if is_login_open() { LoginModal { is_login_open } }
        if is_change_password_open() { ChangePasswordModal { is_change_password_open } }
    ...
    rsx! {
        div { class: "mb-3",
            ...
            div { class: "flex items-center gap-1",
                input { class: "min-w-0 flex-1 bg-base-200 px-2 py-1 text-xs", value: "{address}", readonly: true }
                button { class: "btn btn-ghost btn-square btn-xs", title: "复制地址", onclick: move |_| {
...
fn RoomNavigationItem(
    ...
    rsx! {
        li {
            button { class: "flex items-center gap-3", onclick: move |_| { selected_stream_key.set(stream_key.clone()); },
    ...
    rsx! {
        div { class: "modal modal-open",
            div { class: "modal-box max-w-sm",
                button { class: "btn btn-ghost btn-square btn-sm absolute right-3 top-3", title: "关闭登录", onclick: move |_| is_login_open.set(false), span { class: "icon-[mdi--close] size-5" } }
                ...
                onsubmit: move |event| {
                    ...
                    login_action.call(Form(api::auth::LoginForm { username: login_username, password: login_password }));
                },
                label { class: "fieldset",
                    span { class: "fieldset-legend", "用户名" }
                    input { class: "input w-full", r#type: "text", autocomplete: "username", value: "{username}", oninput: move |event| username.set(event.value()) }
                }
                label { class: "fieldset",
                    span { class: "fieldset-legend", "密码" }
                    input { class: "input w-full", r#type: "password", autocomplete: "current-password", value: "{password}", oninput: move |event| password.set(event.value()) }
                }
            ...
            div { class: "modal-backdrop", onclick: move |_| is_login_open.set(false) }
        }
    }
}
...
fn ChangePasswordModal(mut is_change_password_open: Signal<bool>) -> Element {
    ...
    rsx! {
        div { class: "modal modal-open",
            div { class: "modal-box max-w-sm",
                button { class: "btn btn-ghost btn-square btn-sm absolute right-3 top-3", title: "关闭修改密码", onclick: move |_| is_change_password_open.set(false),
                    span { class: "icon-[mdi--close] size-5" }
                }
                h2 { class: "text-xl font-semibold", "修改密码" }
                form { class: "mt-5 space-y-4", onsubmit: move |event| {
                    event.prevent_default();
                    change_password_action.call(api::auth::ChangePasswordRequest {
                    ...
                    label { class: "fieldset",
                        ...
                        input { class: "input w-full", r#type: "password", autocomplete: "current-password", value: "{current_password}", oninput: move |event| current_password.set(event.value()) }
                    }
                    label { class: "fieldset",
                        span { class: "fieldset-legend", "新密码" }
                        input { class: "input w-full", r#type: "password", autocomplete: "new-password", minlength: "10", value: "{new_password}", oninput: move |event| new_password.set(event.value()) }
                    }
            ...
            div { class: "modal-backdrop", onclick: move |_| is_change_password_open.set(false) }
        }
    }
}
    ...
    rsx! {
        div { class: "modal modal-open",
            div { class: "modal-box max-w-lg",
                button { class: "btn btn-ghost btn-square btn-sm absolute right-3 top-3", title: "关闭创建直播间", onclick: move |_| is_create_room_open.set(false),
                    span { class: "icon-[mdi--close] size-5" }
                }
                h2 { class: "text-xl font-semibold", "创建直播间" }
                if let Some(Ok(room)) = create_account.value() {
                    div { class: "mt-5 space-y-4",
                        p { class: "text-sm text-success", "直播间和房主账号已创建。请立即保存以下信息，密码不会再次显示。" }
                        ReadonlyField { label: "账号名", value: room().owner_username.clone() }
                        ReadonlyField { label: "初始密码", value: room().initial_password.clone() }
                        ReadonlyField { label: "SRT 推流地址", value: room().publish_info.srt_url.clone() }
                        ReadonlyField { label: "RTMP 推流地址", value: room().publish_info.rtmp_url.clone() }
                        div { class: "border-t border-base-300 pt-4 text-sm leading-6 text-base-content/75",
                            ...
                            p { "打开 OBS 的“设置 > 直播”，服务选择“自定义”。将上方 SRT 或 RTMP 推流地址粘贴到“服务器”，优先使用 SRT；保存后点击“开始直播”。" }
                        }
                        button { class: "btn btn-primary w-full", onclick: move |_| is_create_room_open.set(false), "完成" }
                    }
                } else {
                    form { class: "mt-5 space-y-4", onsubmit: move |event| {
                        event.prevent_default();
                        create_account.call(api::rooms::CreateRoomRequest {
                        ...
                        label { class: "fieldset",
                            ...
                            input { class: "input w-full", r#type: "text", autocomplete: "username", minlength: "2", maxlength: "64", value: "{owner_username}", oninput: move |event| owner_username.set(event.value()) }
                        }
                        label { class: "fieldset",
                            span { class: "fieldset-legend", "直播间标题" }
                            input { class: "input w-full", r#type: "text", value: "{title}", oninput: move |event| title.set(event.value()) }
                        }
            ...
            div { class: "modal-backdrop", onclick: move |_| is_create_room_open.set(false) }
        }
    }
}
...
fn ReadonlyField(label: &'static str, value: String) -> Element {
    rsx! {
        label { class: "fieldset",
            ...
            input { class: "input w-full font-mono text-sm", value: "{value}", readonly: true }
        }
    }
}
